...
namespace TableEdit {
	...
	void Simple::FillSelectWhere(QueryStream& WhereClause) {
	}
	...
	std::string Simple::GetSelectQuery() {
		...
		Query << "SELECT " << FieldsClause.str() << std::endl << " FROM " << TablesClause.str() << std::endl;

		selectQueryNextParamId = 1;
		FillSelectWhere(WhereClause);

		std::string where = WhereClause.str();
		if (!where.empty()) {
			Query << " WHERE " << where << std::endl;
		}

		if (HasOrder == true) {
			Query << OrderClause.str() << std::endl;
		}
	...
	void Simple::BindSelectQueryParamTypes(pqxx::prepare::declaration& d) {

	}

	void Simple::BindSelectQueryParams(pqxx::prepare::invocation& inv) {
	...
	std::string Simple::ReadForm() {
		...
		out << th() << "Edit" << th();
		out << tr() << endl;
		//out << GetSelectQuery()+Pagination() << endl;

		try {
			string query = GetSelectQuery() + Pagination();
			pqxx::work w(*DBConnection);
			pqxx::prepare::declaration d(DBConnection->prepare("q", query));
			BindSelectQueryParamTypes(d);
			pqxx::prepare::invocation inv(w.prepared("q"));
			BindSelectQueryParams(inv);
	...
	bool Simple::CatchAJAX() {
		if (GetFormElement(Scope("action")) != "") {
			...
			if (Action == "insert") {
				...
				std::cout << GetFieldByName(GetFormElement("field"))->GetInputHTML("", "", "", "filter-input");
			} else {
				//ReadForm();
			}
